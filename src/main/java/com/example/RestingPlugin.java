...
public class RestingPlugin extends Plugin
{
	...
	public void onMenuEntryAdded(MenuEntryAdded event)
	{
		...
		{
			client.createMenuEntry(-1).setOption("Rest").setTarget(event.getTarget()).setType(MenuAction.RUNELITE).onClick(e ->
			{
	...
	public void onMenuOpened(MenuOpened event)
	{
		...
		{
			client.createMenuEntry(1).setOption("Rest").setTarget(entry.getTarget()).setType(MenuAction.RUNELITE).onClick(e ->
					startRest(player));
		}

		if (event.getFirstEntry().getOption().equals("Spin"))
		{
			event.getFirstEntry().setOption("Rest");
		}
	}
